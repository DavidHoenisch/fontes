...
use crate::model::{
    Annotation, AnnotationKind, Note, NoteAnchor, NoteListEntry, ReadingState, verse_ref_from_id,
};
...
impl Database {
    ...
    pub fn get_note(&self, id: i64) -> Result<Note> {
        let mut stmt = self.user().prepare(
            "SELECT id, title, body, created_at, updated_at FROM note WHERE id = ?1",
        )?;
    ...
    pub fn delete_note(&self, id: i64) -> Result<()> {
        let n = self.user().execute("DELETE FROM note WHERE id = ?1", [id])?;
        if n == 0 {
            return Err(Error::NotFound("note", id.to_string()));
        }
        Ok(())
    ...
    pub fn list_notes(&self) -> Result<Vec<Note>> {
        let mut stmt = self
            .user()
            .prepare("SELECT id, title, body, created_at, updated_at FROM note ORDER BY updated_at DESC")?;
    ...
    pub fn delete_annotation(&self, id: i64) -> Result<()> {
        let n = self.user().execute("DELETE FROM annotation WHERE id = ?1", [id])?;
        if n == 0 {
    ...
    pub fn annotations_for_verses(&self, verse_ids: &[i64]) -> Result<Vec<Annotation>> {
        ...
        let rows = stmt.query_map(params.as_slice(), |row| {
            ...
            let kind = AnnotationKind::parse(&kind_str)
                .ok_or_else(|| rusqlite::Error::InvalidColumnType(1, kind_str, rusqlite::types::Type::Text))?;
            Ok(Annotation {
                id: row.get(0)?,
                kind,
                verse_id: row.get(2)?,
